...
MCEM_Spatial = function(y,x,cens,LI,LS,coords,init.phi,init.nugget,type.sc,kappa,
  ...
  if (show.SE){
    output = list(Theta=output$Theta, theta=output$theta, beta=output$beta, sigma2=output$sigma2,
                  ...
                  Iterations=output$Iterations, time=ptime, X=x, coord=coords, type=type.sc,
                  kappa=kappa)
  } else {
    output = list(Theta=output$Theta, theta=output$theta, beta=output$beta, sigma2=output$sigma2,
...
plot.convergence = function(model){
  ...
  myplot[1:q] = lapply(listabeta, function(.x) ggplot(data.frame(.x[-1]),aes(x=seq(0,Iter),y=.x[-1])) +
                         geom_line(color="dark blue") + labs(x="Iteration", y=bquote(beta[.(.x[1])])) +
                         theme_bw())
  rm(Theta1)
  myplot[[q+1]] = ggplot(data.frame(Theta),aes(x=seq(0,Iter),y=Theta[,q+1])) + geom_line(color="dark blue") + labs(x="Iteration", y=bquote(sigma^2)) + theme_bw()
  myplot[[q+2]] = ggplot(data.frame(Theta),aes(x=seq(0,Iter),y=Theta[,q+2])) + geom_line(color="dark blue") + labs(x="Iteration", y=bquote(phi)) + theme_bw()
  myplot[[q+3]] = ggplot(data.frame(Theta),aes(x=seq(0,Iter),y=Theta[,q+3])) + geom_line(color="dark blue") + labs(x="Iteration", y=bquote(tau^2)) + theme_bw()
